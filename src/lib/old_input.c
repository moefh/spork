...
static struct sp_old_input *new_input(struct sp_mem_pool *pool, enum sp_input_type type)
{
  struct sp_old_input *in = sp_malloc(pool, sizeof(struct sp_old_input));
  if (! in)
    return NULL;
  in->next = NULL;
  in->pool = pool;
...
struct sp_old_input *sp_open_input_file(struct sp_mem_pool *pool, const char *filename, uint16_t file_id, const char *base_filename)
{
  char path[256];
  ...
  struct sp_old_input *in = new_input(pool, SP_INPUT_FILE);
  if (! in) {
    fclose(f);
    return NULL;
  }
...
struct sp_old_input *sp_new_input_string(struct sp_mem_pool *pool, const char *string)
{
  struct sp_old_input *in = new_input(pool, SP_INPUT_STRING);
  if (! in)
    return NULL;
...
void sp_close_input(struct sp_old_input *in)
{
  switch (in->type) {
...
int sp_old_input_fill_buffer_and_next_byte(struct sp_old_input *in)
{
  switch (in->type) {
...
void sp_old_input_unget_byte(struct sp_old_input *in, uint8_t b)
{
